...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kiểm tra tên sự kiện đã tồn tại (trừ sự kiện hiện tại)
    $check = $conn->prepare("SELECT event_id FROM events WHERE event_name = ? AND event_id != ?");
    $check->bind_param("si", $event_name, $event_id);
    $check->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        // Cập nhật sự kiện
        $stmt = $conn->prepare("UPDATE events SET event_name = ?, description = ?, event_date = ?, start_time = ?, end_time = ?, location = ?, max_participants = ?, registration_fee = ? WHERE event_id = ?");
        $stmt->bind_param("ssssssidi", $event_name, $description, $event_date, $start_time, $end_time, $location, $max_participants, $registration_fee, $event_id);
...
<h2 class="section-title">Chỉnh sửa Sự kiện</h2>
...
<div class="shop-container" style="display:flex; gap:20px; align-items:flex-start; padding:20px;">
    ...
    <div class="admin-content" style="flex:1; background:white; padding:20px; border-radius:8px; box-shadow:0 4px 10px rgba(0,0,0,0.1);">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            <h3>Chỉnh sửa Sự kiện: <?php echo htmlspecialchars($event['event_name']); ?></h3>
        ...
        <form method="post" style="max-width:600px;">
            <div style="margin-bottom:15px;">
                ...
                <input type="text" name="event_name" value="<?php echo htmlspecialchars($event['event_name']); ?>" required style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
            </div>

            <div style="margin-bottom:15px;">
                <label style="display:block; margin-bottom:5px; font-weight:600;">Mô tả: *</label>
                <textarea name="description" rows="4" required style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;"><?php echo htmlspecialchars($event['description']); ?></textarea>
            </div>

            <div style="margin-bottom:15px;">
                <label style="display:block; margin-bottom:5px; font-weight:600;">Ngày diễn ra: *</label>
                <input type="date" name="event_date" value="<?php echo $event['event_date']; ?>" required style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
            </div>

            <div style="display:flex; gap:15px; margin-bottom:15px;">
                <div style="flex:1;">
                    <label style="display:block; margin-bottom:5px; font-weight:600;">Giờ bắt đầu:</label>
                    <input type="time" name="start_time" value="<?php echo $event['start_time']; ?>" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div style="flex:1;">
                    <label style="display:block; margin-bottom:5px; font-weight:600;">Giờ kết thúc:</label>
                    <input type="time" name="end_time" value="<?php echo $event['end_time']; ?>" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
            </div>

            <div style="margin-bottom:15px;">
                <label style="display:block; margin-bottom:5px; font-weight:600;">Địa điểm:</label>
                <input type="text" name="location" value="<?php echo htmlspecialchars($event['location']); ?>" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
            </div>

            <div style="display:flex; gap:15px; margin-bottom:15px;">
                <div style="flex:1;">
                    <label style="display:block; margin-bottom:5px; font-weight:600;">Số người tối đa:</label>
                    <input type="number" name="max_participants" value="<?php echo $event['max_participants']; ?>" min="1" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
                <div style="flex:1;">
                    <label style="display:block; margin-bottom:5px; font-weight:600;">Phí đăng ký (VNĐ):</label>
                    <input type="number" name="registration_fee" value="<?php echo $event['registration_fee']; ?>" min="0" step="1000" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                </div>
            </div>
            ...
            <div style="margin-bottom:20px;">
                ...
                <select name="status" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                    <option value="upcoming" <?php echo $event['status'] === 'upcoming' ? 'selected' : ''; ?>>Sắp tới</option>
                    <option value="ongoing" <?php echo $event['status'] === 'ongoing' ? 'selected' : ''; ?>>Đang diễn ra</option>
                    <option value="completed" <?php echo $event['status'] === 'completed' ? 'selected' : ''; ?>>Hoàn thành</option>
                    <option value="cancelled" <?php echo $event['status'] === 'cancelled' ? 'selected' : ''; ?>>Đã hủy</option>
                </select>
            </div>
